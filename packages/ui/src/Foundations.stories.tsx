...
function Swatch({ name, label, kind = "fill" }: { name: string; label: string; kind?: "fill" | "border" | "text" }) {
  return (
    <div className="flex items-center gap-3">
      {kind === "text" ? (
        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-md border border-[var(--nx-border)] bg-[var(--nx-bg)] text-lg font-bold" style={{ color: `var(${name})` }}>Aa</span>
      ) : (
        <span className="h-10 w-10 shrink-0 rounded-md" style={kind === "border" ? { border: `3px solid var(${name})`, background: "var(--nx-surface-2)" } : { background: `var(${name})`, boxShadow: "inset 0 0 0 1px var(--nx-border)" }} />
      )}
      <div className="min-w-0">
        <p className="truncate text-xs font-medium text-[var(--nx-fg)]">{label}</p>
        <p className="truncate font-mono text-[10px] text-[var(--nx-fg-subtle)]">{name}</p>
        <p className="truncate font-mono text-[10px] text-[var(--nx-fg-subtle)]">{val(name) || "—"}</p>
      </div>
    </div>
  );
}
...
function H({ children }: { children: string }) {
  return <h3 className="mb-3 mt-7 text-sm font-semibold text-[var(--nx-fg)] first:mt-0">{children}</h3>;
}
...
export const Colors: Story = {
  render: () => (
    <div className="max-w-4xl text-[var(--nx-fg)]">
...
export const RadiusAndMotion: Story = {
  name: "Radius & motion",
  render: () => (
    <div className="max-w-3xl text-[var(--nx-fg)]">
      <H>Radius</H>
      <div className="flex flex-wrap gap-6">
        {([["--nx-radius-sm", "sm"], ["--nx-radius", "base"], ["--nx-radius-lg", "lg"]] as [string, string][]).map(([n, l]) => (
          <div key={n} className="text-center">
            <div className="h-16 w-16 border-2 border-[var(--nx-border-strong)] bg-[var(--nx-surface-2)]" style={{ borderRadius: `var(${n})` }} />
            <p className="mt-2 text-xs">{l}</p>
            <p className="font-mono text-[10px] text-[var(--nx-fg-subtle)]">{n}</p>
          </div>
        ))}
      </div>
      <H>Motion</H>
      <div className="grid grid-cols-3 gap-3">
        {([["--nx-motion-fast", "micro"], ["--nx-motion-base", "state"], ["--nx-motion-slow", "large"]] as [string, string][]).map(([n, l]) => (
          <div key={n} className="rounded-md border border-[var(--nx-border)] bg-[var(--nx-surface-1)] p-3">
            <p className="font-mono text-xs">{val(n) || "—"}</p>
            <p className="text-[11px] text-[var(--nx-fg-muted)]">{l}</p>
            <p className="font-mono text-[10px] text-[var(--nx-fg-subtle)]">{n}</p>
          </div>
        ))}
      </div>
      ...
      {Array.from({ length: 12 }, (_, i) => i + 1).map((i) => {
        ...
        return (
          <div key={i} style={{ width: 64 }}>
            <div style={{ height: 48, borderRadius: 8, background: `var(${name})`, boxShadow: "inset 0 0 0 1px var(--nx-border)" }} />
...
export const PrimitiveColors: Story = {
  ...
  render: () => (
    <div className="max-w-4xl text-[var(--nx-fg)]">
      ...
      <p className="mb-3 text-[11px] text-[var(--nx-fg-subtle)]">Surfaces alias 1–4, borders 6–7, text 9/10/12.</p>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {SIZES.map((s) => (
          <div key={s} style={{ display: "flex", alignItems: "baseline", gap: 16 }}>
            <code style={{ width: 130, flexShrink: 0, fontFamily: "var(--nx-font-mono)", fontSize: 11, color: "var(--nx-fg-subtle)" }}>--nx-text-{s}</code>
            <span style={{ fontSize: `var(--nx-text-${s})`, color: "var(--nx-fg)" }}>Aa</span>
          </div>
        ))}
      </div>
